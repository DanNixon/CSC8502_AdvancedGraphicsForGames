...
Matrix4 Renderer::GenerateInverseView(Camera &c)
{
  ...
  Matrix4 iview = Matrix4::Translation(position) * Matrix4::Rotation(yaw, Vector3(0, -1, 0)) *
...
Vector3 Renderer::GetMouseDirectionVector3(float aspect, float fov, Camera &cam)
{
  ...
  Vector3 nearPos = Vector3(mpos.x, height - mpos.y, 0.0f);

  // We also don't use exactly 1.0 (the normalised 'end' of the far plane) as this
  // causes the unproject function to go a bit weird.
  Vector3 farPos = Vector3(mpos.x, height - mpos.y, 0.99999999f);
