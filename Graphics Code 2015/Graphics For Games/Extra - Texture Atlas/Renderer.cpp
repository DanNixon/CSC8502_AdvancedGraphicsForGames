...
/*
Like vertex attributes and lights etc, it's best to have a single function to update the atlas
shader.
As long as we always use the same uniform names when we use atlasing in a vertex shader, this'll
'just work'. We can keep all of the data we need to perform vertex atlasing in a Vector4 for the
node,
...
{
  glUniform3fv(glGetUniformLocation(currentShader->GetProgram(), "atlasInfo"), 1,
               (float *)&node->GetAtlas()->PackValuesForShader());
  glUniform4fv(glGetUniformLocation(currentShader->GetProgram(), "textureInfo"), 1,
               (float *)&node->PackValuesForShader());
}