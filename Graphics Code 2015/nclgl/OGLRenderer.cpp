    ...
    : m_parent(window)
{
  ...
  // A pixel format descriptor is a struct that tells the Windows OS what type of front / back
  // buffers we want to create etc
  ...
  pfd.dwFlags = PFD_DOUBLEBUFFER | PFD_SUPPORT_OPENGL |
                PFD_DRAW_TO_WINDOW; // It must be double buffered, it must support OGL(!), and it
                                    // must allow us to draw to it...
  ...
  HGLRC tempContext; // We need a temporary OpenGL context to check for OpenGL 3.2
  ...
  int attribs[] = {
      ...
      WGL_CONTEXT_CORE_PROFILE_BIT_ARB, // We want everything OpenGL 3.2 provides...
      0                                 // That's enough attributes...
  };

  // Everywhere else in the Renderers, we use function pointers provided by GLEW...but we can't
  // initialise GLEW yet! So we have to use the 'Wiggle' API
  ...
  glewExperimental = GL_TRUE; // This forces GLEW to give us function pointers for everything (gets
                              // around GLEW using 'old fashioned' methods
...
// If we get this far, everything's going well!
...
#ifdef OPENGL_DEBUGGING
  // PFNWGLCREATECONTEXTATTRIBSARBPROC glDebugMessageCallbackTEMP =
  // (PFNWGLCREATECONTEXTATTRIBSARBPROC) wglGetProcAddress("glDebugMessageCallbackARB");
  ...
  window.SetRenderer(this); // Tell our window about the new renderer! (Which will in turn resize
                            // the renderer window to fit...)
                                  ...
                                  const Vector3 &colour)
{
  ...
  for (int i = 0; i < stepCount; ++i)
  {
    ...
    target->AddLine(Vector3(startx, starty, 0), Vector3(endx, endy, 0), colour, colour);
  }
}
