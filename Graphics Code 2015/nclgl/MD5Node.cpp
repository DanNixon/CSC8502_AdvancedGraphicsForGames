...
void MD5Node::DebugDrawSkeleton()
{
  ...
  */
  for (int i = 0; i < currentSkeleton.numJoints; ++i)
  {
    ...
    {
      skeletonVertices[(i * 2) + 1] =
          currentSkeleton.joints[currentSkeleton.joints[i].parent].transform.GetPositionVector();
      ;
    }
  }

  // You should know what this all does by now, except we combine it with the draw operations in a
  // single function
  ...
  // Draws the array twice, once as points, and once as lines. glLineWidth may or may not actually
  // do anything
